...
export default function SocialLinks({ variant = 'icon-only', className = '' }: SocialLinksProps) {
  if (variant === 'with-label') {
    return (
      <div className={`social-links-labeled ${className}`}>
        <a href={INSTAGRAM} target="_blank" rel="noopener noreferrer" className="social-chip instagram">
          <InstagramIcon />
          <span>Instagram</span>
        </a>
        <a href={FACEBOOK} target="_blank" rel="noopener noreferrer" className="social-chip facebook">
          <FacebookIcon />
          <span>Facebook</span>
        </a>
      </div>
